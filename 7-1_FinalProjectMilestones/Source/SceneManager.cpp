...
SceneManager::SceneManager(ShaderManager *pShaderManager)
	: m_loadedTextures(0) // Initialize it to 0
{
 ...
 *
 *  Configures material properties for objects in the scene,
 *  including:
 *  - Gold
 *  - Cement
 *  - Wood
 *  - Glass
 *  - Sunlight (Global Lighting Effect)
 ***********************************************************/
void SceneManager::DefineObjectMaterials()
{
	...
	// Sunlight material (For objects that primarily reflect sunlight)
	OBJECT_MATERIAL sunlightMaterial;
	sunlightMaterial.ambientColor = glm::vec3(0.8f, 0.8f, 0.6f);
	sunlightMaterial.ambientStrength = 0.5f;
	sunlightMaterial.diffuseColor = glm::vec3(1.0f, 1.0f, 0.8f);
	sunlightMaterial.specularColor = glm::vec3(1.0f, 1.0f, 1.0f);
	sunlightMaterial.shininess = 90.0;
...
void SceneManager::SetupSceneLights()
{
	...
	m_pShaderManager->setVec3Value("sunLight.direction", -0.2f, -1.0f, -0.3f); // Slightly angled down
	m_pShaderManager->setVec3Value("sunLight.ambientColor", 0.3f, 0.3f, 0.3f);
	m_pShaderManager->setVec3Value("sunLight.diffuseColor", 0.8f, 0.8f, 0.6f); // Warm sunlight tone
	m_pShaderManager->setVec3Value("sunLight.specularColor", 0.5f, 0.5f, 0.5f);

	// Primary White Light (Point Light)
	...
	// Soft White Light (Point Light)
	m_pShaderManager->setVec3Value("lightSources[1].position", 0.6f, 5.0f, 6.0f);
	...
	// Debug: Add a small ambient light to prevent full darkness
	m_pShaderManager->setVec3Value("globalAmbient", 0.3f, 0.3f, 0.3f);
}
...
void SceneManager::RenderScene()
{
	...
	SetShaderColor(0.0f, 0.5f, 0.0f, 1.0f);
	SetShaderTexture("root");
...
// set the XYZ scale for the mesh
	...
	SetShaderColor(0.6f, 0.3f, 0.0f, 1.0f);
	SetShaderTexture("root");
	SetShaderMaterial("wood");
